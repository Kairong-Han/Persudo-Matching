...
if not is_load:
    ...
    np.savetxt('X_train_fusion.txt', X_train_fusion)
    np.savetxt('Y_train_fusion.txt', Y_train_fusion)
    np.savetxt('T_train_fusion.txt', T_train_fusion)
else:
    X_train_fusion = np.loadtxt('X_train_fusion.txt')
    Y_train_fusion = np.loadtxt('Y_train_fusion.txt')
    T_train_fusion = np.loadtxt('T_train_fusion.txt')
...
# 分离治疗组 (T=1) 和 对照组 (T=0) 的 X
# 设置图像大小


# draw_plt(X_train,T_train)
# draw_plt(X_train,Tb_train)
...
model_golden = TARNet(input_dim, hidden_dim)
model_f = TARNet(input_dim, hidden_dim)

# 定义损失函数和优化器
criterion = nn.BCELoss()
optimizer_b = optim.Adam(model_b.parameters(), lr=0.0005)
optimizer_g = optim.Adam(model_golden.parameters(), lr=0.0005)
...
train_model(model_golden, X_train_tensor, Y_train_tensor, T_train_tensor,optimizer_g,scheduler_g)
...
model_golden.eval()
with torch.no_grad():
    treatment_outcome_test_g, control_outcome_test_g = model_golden(X_test_tensor)
...
HTE_test_g = (treatment_outcome_test_g - control_outcome_test_g).squeeze().numpy()
...
print(f"bias Qini coefficient by fusion: {qini_coef:.4f} mape_copc {qini_mape_copc(Yb_test, Tb_test, HTE_test_f)}")
...
model_golden.eval()
with torch.no_grad():
    treatment_outcome_test_g, control_outcome_test_g = model_golden(X_obs)
...
qini_coef = qini_auc_score(y_true=Y_g, uplift=HTE_test_g, treatment=T_g)
mape,copc = qini_mape_copc(Y_g, T_g, HTE_test_g)
with open(log_file,'a+') as f:
    f.write(f'{qini_coef}, {mape}, {copc}')
    f.write('\n')
print(f"ground truth Qini coefficient by golden: {qini_coef:.4f} mape_copc {qini_mape_copc(Y_g, T_g, HTE_test_g)}")
...
print(f"ground truth Qini coefficient by fusion: {qini_coef:.4f} mape_copc {qini_mape_copc(Y_g, T_g, HTE_test_f)}")
from sklift.viz import plot_uplift_curve
# fig, ax = plt.subplots(1, 1)
# ax.set_title('Uplift curves')

# plot_uplift_curve(
#     Y_test, HTE_test, T_test,
#     perfect=True, ax=ax
# );
# plt.show()

# plot_uplift_curve(
#     Y_test, HTE_test_f, T_test,
#     perfect=True, ax=ax
# );
# plt.show()
#
