...
from util import CausalForestModel
...
model_b = CausalForestModel(input_dim, hidden_dim)
model_b.fit(X_train_tensor, Tb_train_tensor,Yb_train_tensor )
HTE_test = model_b.predict(X_test_tensor)
...
HTE_test = model_b.predict(X_obs)
...
for rate in [1,3,5]:
    for mode in ['ours','random']:
        ...
        model_f = CausalForestModel(input_dim, hidden_dim)
        # 定义损失函数和优化器

        model_f.fit(X_train_fusion_tensor,T_train_fusion_tensor,Y_train_fusion_tensor)
        # 训练模型
        ...
        HTE_test_f = model_f.predict(X_test_tensor)
        ...
        HTE_test_f = model_f.predict(X_obs)
